...
animalHealthDataset = make_clean(animalHealthConditionDataset)
...
healthTrain, healthTest, healthLabelTrain, healthLabelTest = train_test_split(
    animalHealthConditionDataset.iloc[:, :6], animalHealthConditionDataset.iloc[:, -1], train_size=.75, random_state=5
)
...
animalHealthPipeline.fit(healthTrain, healthLabelTrain)  # training animal health model
animalDiseasePipeline.fit(diseaseTrain, diseaseTrainLabel)  # training animal disease predictor model
